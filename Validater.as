package {
	...
	public class Validater extends Sprite {
		...
		//private var a0:Array = [120,-38,-117,-53,40,41,41,-48,-88,41,-42,-76,-46,-41,-41,-120,41,-41,-46,-77,79,-73,-80,52,-48,75,-50,-49,-83,1,-13,114,51,-117,18,-45,51,-13,82,65,34,-102,0,77,-119,14,-58];
		private var a1:Array = [120,-38,-53,-55,79,78,44,-55,-52,-49,-45,-53,40,74,77,-45,43,-55,15,46,41,-54,-52,75,7,0,100,37,8,-75];
		...
		function Validater(parent:DisplayObjectContainer) {
			...
			lang = Capabilities.language == "zh-CN" ? AtoString(a3) : AtoString(a2);
		}
		
		public function fromNiamod(tar:Array):void {
			if (!parentStage) return;
			
			var url:String = parentStage.loaderInfo.url;
			//try {
				//url = ExternalInterface.call(AtoString(a1));
			//}catch (e:Error) {
				//trace(e);
			//}
			trace(url); //^https?://(.+\.)?[g890\.com|miragine\.com]/
			var langCheck:RegExp = new RegExp(AtoString(tar), "i");
			if (!langCheck.test(url)){
				showString();
			}
		}
		
		public function fromLRU(tar:Array):void {
			if (!parentStage) return;
			
			var cst:int = 10;
			var lru:String = AtoString(tar);
			checkLRU(lru);
			
			function checkLRU(str:String):void {
		...
		private function showString():void {
			...
			if(Capabilities.language == "zh-CN"){
				navigateToURL(new URLRequest("http://cn.miragine.com"), "_blank"); 
			}else {
				navigateToURL(new URLRequest("http://en.miragine.com"), "_blank"); 
			}
		}
